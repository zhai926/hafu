...
/**
 * @version 2010-03-31 樊晓东 <BR>
 *          VSFtp操作类，对远程的VSFtp服务器进行目录和文件的创建操作
 */
public class VSFtpUtils {

	private static final Logger logger = Logger.getLogger(VSFtpUtils.class);
	...
	public static void setFtpport(int ftpport) {
		VSFtpUtils.ftpport = ftpport;
	}

	public static void setFtppath(String ftppath) {
		VSFtpUtils.ftppath = ftppath;
	}
	...
	public static boolean defaultPut(File file,String picName,String relativePath,int gwidth){
		...
		VSFtpUtils.setFtppath(remoteFilePath);
		return put(file, picName, relativePath, gwidth);
	}

	/**
	 * VSFtp文件上传方法，在远程VSFtp服务器上创建所需的目录及文件
	...
	public static void main(String[] args) {
		...
		VSFtpUtils.setFtpport(Integer.parseInt(port));
		VSFtpUtils.setFtppath(remoteFilePath);			
		VSFtpUtils.put(new File("c:\\1.jpg"), "test" + new Date().getTime()+".jpg", "/test", 0);
